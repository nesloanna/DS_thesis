...
for i in range(1, len(t)):
    sst[i] = a*sst[i-1] + 0.75*np.random.randn() + 0.5 * \
        np.cos(t[i]*2*np.pi/365.25)
sst = sst - sst.min() + 5.
...
plt.bar(range(mhws['n_events']), mhws['duration'],
        width=0.6, color=(0.7, 0.7, 0.7))
...
plt.bar(range(mhws['n_events']), mhws['intensity_max'],
        width=0.6, color=(0.7, 0.7, 0.7))
...
plt.bar(range(mhws['n_events']), mhws['intensity_mean'],
        width=0.6, color=(0.7, 0.7, 0.7))
...
plt.bar(range(mhws['n_events']), mhws['intensity_cumulative'],
        width=0.6, color=(0.7, 0.7, 0.7))
...
plt.xlabel(mhwname + ' event number')
plt.savefig('mhw_stats/' + mhwname + '_list_byNumber.png',
...
plt.bar(mhws['date_peak'][evMax], mhws['duration']
        [evMax], width=150, color=col_bar)
plt.xlim(ts, te)
plt.ylabel('[days]')
plt.title('Duration')
plt.subplot(2, 2, 2)
evMax = np.argmax(np.abs(mhws['intensity_max']))
plt.bar(mhws['date_peak'], mhws['intensity_max'],
        width=150, color=(0.7, 0.7, 0.7))
plt.bar(mhws['date_peak'][evMax], mhws['intensity_max']
        [evMax], width=150, color=col_bar)
plt.xlim(ts, te)
plt.ylabel(r'[$^\circ$C]')
plt.title('Maximum Intensity')
plt.subplot(2, 2, 4)
evMax = np.argmax(np.abs(mhws['intensity_mean']))
plt.bar(mhws['date_peak'], mhws['intensity_mean'],
        width=150, color=(0.7, 0.7, 0.7))
plt.bar(mhws['date_peak'][evMax], mhws['intensity_mean']
        [evMax], width=150, color=col_bar)
plt.xlim(ts, te)
plt.title('Mean Intensity')
plt.ylabel(r'[$^\circ$C]')
plt.subplot(2, 2, 3)
evMax = np.argmax(np.abs(mhws['intensity_cumulative']))
plt.bar(mhws['date_peak'], mhws['intensity_cumulative'],
        width=150, color=(0.7, 0.7, 0.7))
plt.bar(mhws['date_peak'][evMax], mhws['intensity_cumulative']
        [evMax], width=150, color=col_bar)
plt.xlim(ts, te)
plt.title(r'Cumulative Intensity')
plt.ylabel(r'[$^\circ$C$\times$days]')
plt.savefig('mhw_stats/' + mhwname + '_list_byDate.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
for i in range(10):
    ...
    for ev0 in np.arange(max(ev-10, 0), min(ev+11, mhws['n_events']-1), 1):
        ...
        plt.fill_between(dates[t1:t2+1], sst[t1:t2+1],
                         clim['thresh'][t1:t2+1], color=col_ev)
    ...
    plt.fill_between(dates[t1:t2+1], sst[t1:t2+1],
                     clim['thresh'][t1:t2+1], color=col_evMax)
    ...
    if coldSpells:
        plt.ylim(clim['seas'].min() + mhws['intensity_max']
                 [ev] - 0.5, clim['seas'].max() + 1)
    else:
    ...
    outfile.write('Maximum intensity: ' +
                  str(mhws['intensity_max'][ev]) + ' deg. C\n')
    outfile.write('Average intensity: ' +
                  str(mhws['intensity_mean'][ev]) + ' deg. C\n')
    outfile.write('Cumulative intensity: ' +
                  str(mhws['intensity_cumulative'][ev]) + ' deg. C-days\n')
    outfile.write('Duration: ' + str(mhws['duration'][ev]) + ' days\n')
    outfile.write('Start date: ' +
                  str(mhws['date_start'][ev].strftime("%d %B %Y")) + '\n')
    outfile.write(
        'End date: ' + str(mhws['date_end'][ev].strftime("%d %B %Y")) + '\n')
    outfile.write('\n')

plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
outfile.close()
plt.savefig('mhw_stats/' + mhwname + '_topTen_iMax.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
outfile.close()
plt.savefig('mhw_stats/' + mhwname + '_topTen_iMean.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
outfile.close()
plt.savefig('mhw_stats/' + mhwname + '_topTen_iCum.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
outfile.close()
plt.savefig('mhw_stats/' + mhwname + '_topTen_Dur.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
if np.abs(trend['count']) - dtrend['count'] > 0:
    plt.title('Frequency (trend = ' +
              '{:.2}'.format(10*trend['count']) + '* per decade)')
else:
    plt.title('Frequency (trend = ' +
              '{:.2}'.format(10*trend['count']) + ' per decade)')
...
if np.abs(trend['duration']) - dtrend['duration'] > 0:
    plt.title(
        'Duration (trend = ' + '{:.2}'.format(10*trend['duration']) + '* per decade)')
else:
    plt.title(
        'Duration (trend = ' + '{:.2}'.format(10*trend['duration']) + ' per decade)')
plt.ylabel('[days]')
...
if (np.abs(trend['intensity_max']) - dtrend['intensity_max'] > 0) * (np.abs(trend['intensity_mean']) - dtrend['intensity_mean'] > 0):
    plt.title('Intensity (trend = ' + '{:.2}'.format(10*trend['intensity_max']) + '* (max), ' + '{:.2}'.format(
        10*trend['intensity_mean']) + '* (mean) per decade)')
...
if np.abs(trend['intensity_cumulative']) - dtrend['intensity_cumulative'] > 0:
    plt.title('Cumulative intensity (trend = ' +
              '{:.2}'.format(10*trend['intensity_cumulative']) + '* per decade)')
else:
    plt.title('Cumulative intensity (trend = ' +
              '{:.2}'.format(10*trend['intensity_cumulative']) + ' per decade)')
plt.ylabel(r'[$^\circ$C$\times$days]')
plt.grid()
plt.savefig('mhw_stats/' + mhwname + '_annualAverages_meanTrend.png',
            bbox_inches='tight', pad_inches=0.5, dpi=150)
...
csvfile.write('# Marine heatwave statistics for individual detected events at [' + str(
    lon[i]) + ' E ' + str(lat[j]) + ' N] from NOAA OI AVHRR V2 SST data (1982-2014)\n')
# csvfile.write('# Marine ' + mhwfullname + ' statistics for individual detected events at ' +
#               locations['name'][0] + ' from NOAA OI AVHRR V2 SST data (1982-2014)\n')
...
for ev in range(mhws['n_events']):
    csvfile.write(str(ev+1) + ', ' + str(mhws['date_start'][ev].year) + ', ' + str(mhws['date_start'][ev].month) + ', ' + str(mhws['date_start'][ev].day) + ', ' + str(mhws['date_peak'][ev].year) + ', ' + str(mhws['date_peak'][ev].month) + ', ' + str(mhws['date_peak'][ev].day) + ', ' + str(mhws['date_end'][ev].year) + ', ' + str(mhws['date_end'][ev].month) + ', ' + str(mhws['date_end'][ev].day) + ', ' + str(mhws['duration'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_max'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_mean'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_cumulative'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_var'][ev]) + ', ' + '{:.4}'.format(
        mhws['rate_onset'][ev]) + ', ' + '{:.4}'.format(mhws['rate_decline'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_max_relThresh'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_mean_relThresh'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_cumulative_relThresh'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_var_relThresh'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_max_abs'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_mean_abs'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_cumulative_abs'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_var_abs'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_max_norm'][ev]) + ', ' + '{:.4}'.format(mhws['intensity_mean_norm'][ev]) + '\n')
csvfile.close()

# Annual average data
csvfile = open(outfile + '.annual.csv', 'w')
csvfile.write('# Annual average marine heatwave statistics at [' + str(
    lon[i]) + ' E ' + str(lat[j]) + ' N] from NOAA OI AVHRR V2 SST data (1982-2014)\n')
# csvfile.write('# Annual average marine ' + mhwfullname + ' statistics at ' +
#               locations['name'][0] + ' from NOAA OI AVHRR V2 SST data (1982-2014)\n')
csvfile.write(
    '# A value of nan indicates missing data. This should correspond to a year with no MHW events (count = 0)\n')
csvfile.write('Year, ' + mhwname + ' event count [number], Duration [days], Maximum intensity [deg C], Mean intensity [deg C], Cumulative intensity [deg C x days], Total ' + mhwname +
              ' days [days], Total cumulative intensity [deg C x days], Intensity variability [deg C], Rate of onset [deg C / days], Rate of decline [deg C / days], Maximum intensity (rel. thresh.) [deg C], Mean intensity (rel. thresh.) [deg C], Cumulative intensity (rel. thresh.) [deg C x days], Intensity variability (rel. thresh.) [deg C], Maximum intensity (absolute) [deg C], Mean intensity (absolute) [deg C], Cumulative intensity (absolute) [deg C x days], Intensity variability (absolute) [deg C], Maximum intensity (normalized) [unitless], Mean intensity (normalized) [unitless], Mean temperature [deg C], Max temperature [deg C], Min temperature [deg C]\n')
for yr in range(len(mhwBlock['years_centre'])):
    csvfile.write(str(mhwBlock['years_centre'][yr].astype(int)) + ', ' + str(mhwBlock['count'][yr].astype(int)) + ', ' + '{:.4}'.format(mhwBlock['duration'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_max'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_mean'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_cumulative'][yr]) + ', ' + '{:.4}'.format(mhwBlock['total_days'][yr]) + ', ' + '{:.4}'.format(mhwBlock['total_icum'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_var'][yr]) + ', ' + '{:.4}'.format(mhwBlock['rate_onset'][yr]) + ', ' + '{:.4}'.format(mhwBlock['rate_decline'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_max_relThresh'][yr]) + ', ' + '{:.4}'.format(
        mhwBlock['intensity_mean_relThresh'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_cumulative_relThresh'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_var_relThresh'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_max_abs'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_mean_abs'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_cumulative_abs'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_var_abs'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_max_norm'][yr]) + ', ' + '{:.4}'.format(mhwBlock['intensity_mean_norm'][yr]) + ', ' + '{:.4}'.format(mhwBlock['temp_mean'][yr]) + ', ' + '{:.4}'.format(mhwBlock['temp_max'][yr]) + ', ' + '{:.4}'.format(mhwBlock['temp_min'][yr]) + '\n')
csvfile.close()
