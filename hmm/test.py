...
# Create dataframes with the extra sample IDs
# df_meso_extra = df_meso_SL[df_meso_SL['Sample ID (TARA_barcode#)'].isin(
#     unique_ids_meso)]
# df_bio_extra = df_bio[df_bio['Sample ID (TARA_barcode#)'].isin(
#     unique_ids_depth)]
# df_depth_extra = df_depth[df_depth['Sample ID (TARA_barcode#)'].isin(
#     unique_ids_depth)]

print("Original shape of meso:", df_meso_SL.shape)

# Remove rows containing the values in the set
# df_meso_SL = df_meso_SL[~df_meso_SL['Sample ID (TARA_barcode#)'].isin(
#     unique_ids_meso)]
# df_bio = df_bio[~df_bio['Sample ID (TARA_barcode#)'].isin(unique_ids_depth)]
# df_depth = df_depth[~df_depth['Sample ID (TARA_barcode#)'].isin(
#     unique_ids_depth)]

# print("New shape of meso:", df_meso_SL.shape)
# print("New shape of bio:", df_bio.shape)
# print("New shape of depth:", df_depth.shape)
...
# new_ids_for_meso = sorted(list(unique_ids_depth))
# df_new_meso_ids = pd.DataFrame({'Sample ID (TARA_barcode#)': new_ids_for_meso})
# df_meso_SL = pd.concat([df_meso_SL, df_new_meso_ids], ignore_index=True)
...
# df_meso_SL = df_meso_SL.fillna("?")
# df_bio = df_bio.fillna("?")
# df_depth = df_depth.fillna("?")
...
# merged_column = pd.concat([df_depth['Sample ID (TARA_barcode#)'],
#                            df_meso_SL['Sample ID (TARA_barcode#)']],
#                           ignore_index=True, verify_integrity=True, join='inner')
